...
如同var宣告的變數，函式宣告式也會被懸掛在範圍最上面，讓你可在函式被宣告前呼叫它。

但指派給變數的函式運算式**不會**被懸掛，因他們受到變數範圍規則的約束。

```
f();                  // TypeError: f is not a function
let f = function() {  // var也一樣，會有TypeError的問題
  console.log('f');
}
```
...
(temporal dead zone, TDZ)，意思是使用`let`宣告的變數在你宣告它之前不會存在。在範圍內，變數的TDZ是變數被宣告之前的程式碼。

可能會讓熟悉JavaScript ES6之前的版本的使用者犯錯。

`typeof`運算子通常用來判斷變數是否已被宣告，被視為一種測試存在與否的"安全"方式。原本在let與TDZ之前是安全的做法，但用`let`來宣告就不安全了。
...
ES6就比較不需要用`typeof`來檢查變數是否存在。

## 嚴格模式

ES5語法可編寫所謂的**隱式全域**，這非常令人崩潰的程式錯誤根源。簡單來說，忘了用var宣告變數，JavaScript會開心假設你指的是一個全域變數，如果該全域變數不存在，它會為你建議一個!

所以基於這個(or other)原因，JavaScript加入一個**嚴格模式**的概念，可防止隱式全域。

在其他程式碼之前使用"use strict"啟用。

- 全域中使用，整個程式碼都是嚴格模式(不建議，現代網站都會將許多程式組在一起)
- 函式中使用，該函式將執行嚴格模式

如果不想在寫的每個函式都啟用嚴格模式(誰想?)，那就用IIFE包起來你的所有程式。

```
(function() {
  'use strict';
  // ....該模式就不影響其他與這段程式碼結合的指令碼
